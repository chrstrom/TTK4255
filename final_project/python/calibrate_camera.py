...
if exists(join(output_folder, "u_all.npy")):
    ...
    X_board[:, :2] = square_size * np.mgrid[
    ...
    for image_path in glob.glob(image_path_pattern):
        ...
        ok, u = cv.findChessboardCorners(
            I, (board_size[0], board_size[1]), detect_flags
        )
        if ok:
    ...
    np.save(
        join(output_folder, "u_all.npy"), u_all
    )  # Detected checkerboard corner locations
    np.save(
        join(output_folder, "X_all.npy"), X_all
    )  # Corresponding 3D pattern coordinates

print("Calibrating. This may take a minute or two...", end="")
results = cv.calibrateCameraExtended(
    X_all, u_all, image_size, None, None, flags=calibrate_flags
)
print("Done!")
...
np.savetxt(
    join(output_folder, "std_int.txt"), std_int
)  # Standard deviations of intrinsics (entries in K and distortion coefficients)
