...
def derivative_of_gaussian(I, sigma):
    ...
    for row in range(I.shape[0]):
        Ix[row, :] = np.convolve(I[row, :], d, mode="same")
    for col in range(I.shape[1]):
        Ix[:, col] = np.convolve(Ix[:, col], g, mode="same")
    for col in range(I.shape[1]):
        Iy[:, col] = np.convolve(I[:, col], d, mode="same")
    for row in range(I.shape[0]):
        Iy[row, :] = np.convolve(Iy[row, :], g, mode="same")
...
def extract_local_maxima(H, threshold):
    ...
    )  # Threshold is specified as fraction of maximum array value
